...
/*
 *	foodriver_client.c - pseudo-MIG client side for the sound/dsp driver.
 *	1.0 version generated by Lee Boynton and Gregg Kellogg
 *	Copyright 1988-90 NeXT, Inc.
 *
 * New entry points for release 2.0:
 *	foodriver_dsp_reset
 *	foodriver_new_device_port
 *	foodriver_dspcmd_abortdma
 * 	foodriver_set_dsp_buffers_per_soundout_buffer
 *	foodriver_dsp_dma_write
 *	foodriver_dsp_dma_read
 *	foodriver_dsp_read_data
 *	foodriver_dsp_read_messages
 *	foodriver_set_ramp
 *	foodriver_reset_dsp_owner
 *	foodriver_reset_sndin_owner
 *	foodriver_reset_sndout_owner
 *	foodriver_set_sndout_bufsize
 *	foodriver_set_sndout_bufcount
 *	foodriver_stream_ndma
 *
 * Modification history
 * 03/10/90/jos - Tested commented system panic by moving stream_options 
 *		  setting before record/write in 
 *		  foodriver_start_{reading,writing} instead of after.
 *		  This caused a panic as advertised, so I put them back.
 *		- added setting of stream options to 
 *		  case FOODRIVER_STREAM_DSP_TO_SNDOUT_{22,44} in stream_setup.
 ...
 * 		- Added foodriver_set_dsp_buffers_per_soundout_buffer().
 ...
 * 		- Flushed FOODRIVER_DSP_PROTO_SOUNDDATA bit which was being used
 *		  to bypass DMA.
 *		- added foodriver_dspcmd_chandata() for specifying DSP side
 ...
 * 06/07/90/jos - Flushed chandata args from foodriver_dsp_dma_{read,write}.
 * 06/07/90/jos - Moved *_PROTO_C_DMA and *_PROTO_S_DMA protocol bit defs to 
 *		  foodriver.h. They are needed when setting the protocol.
 ...
 * 06/18/90/jos - change foodriver_dsp{cmd_req_condition,_set_flags,
 ...
 * 07/14/90/jos - added foodriver_dsp_read_messages()
 *		  changed foodriver_dsp_read() to "do the right thing"
 *		  whatever the dsp protocol.
 * 07/17/90/mtm - In foodriver_stream_setup(), set SND_DSP_PROTO_HIGH in
 *		  FOODRIVER_DMA_STREAM_THROUGH_DSP_TO_SNDOUT_44 case.
 * 07/17/90/mtm	- Fix chandata setup for above case.
 * 07/18/90/mtm - Made all static message const.
 * 08/05/90/jos - Fixed infinite port_allocate bug, foodriver_dsp_read_messages
 ...
 * 09/18/90/mtm	- Add foodriver_reset_{dsp,sndin,sndout}_owner() (bug #9400).
 * 09/24/90/mtm - Make all messages rpc's with timeouts (bug #4673).
 * 09/26/90/mtm - Add foodriver_set_sndout_buf{size,count} and
 *		  foodriver_stream_ndma, removed
 *		  foodriver_set_dsp_buffers_per_soundout_buffer()
 *		  (bug #7727 and 7728).
 ...
 Remove foodriver_set_sndout_{bufsize,bufcount}, replace with generic
  foodriver_set_stream_param(stream,param).
 Replace foodriver_stream_setup() with var_args function switched on configuration.
...
static int dsp_protocol = 0;	/* saved by foodriver_dsp_proto() */
...
#if 0
THIS IS KNOWN TO BREAK foodriver_dsp_boot()!  See FIXME below.
/*
...
static kern_return_t foodriver_set_owner (
	port_t		device_port,
	port_t		owner_port,
	port_t		*neg_port,
	int		msg_id)
...
{
	return foodriver_set_owner(device_port, owner_port, neg_port,
	...
	int		priority)		// priority of this transaction
{
	...
	while (count) {
		...
		default:
#if DPRINT
		    fprintf(stderr,"*** foodriver_dsp_read_data: "
...
#if DPRINT
			fprintf(stderr,
				"\t(foodriver_dsp_read_data: ill_msgid: %d)\n",
				((snd_illegal_msg_t *)msg)->ill_msgid);
			fprintf(stderr,
				"\t(foodriver_dsp_read_data: error: %d)\n",
...
/* In 1.0, the following was named foodriver_dsp_read(). */
	...
	int		priority)		// priority of this transaction
{
	...
	while (remaining_count > 0) {
		...
		err = foodriver_dspcmd_req_msg(cmd_port, reply_port);
		if (err) return err;
		msg->msg_size = MSG_SIZE_MAX;
...
{
    if (   
	!(dsp_protocol & FOODRIVER_DSP_PROTO_RAW) /* Raw mode */
	|| dsp_protocol & FOODRIVER_DSP_PROTO_DSPMSG /* DSP messages */
	|| dsp_protocol & FOODRIVER_DSP_PROTO_DSPERR /* DSP error messages */
	|| dsp_protocol & FOODRIVER_DSP_PROTO_C_DMA ) /* Complex DMA mode */
      return foodriver_dsp_read_messages(cmd_port,data,count,
					 data_size,priority);
    else			/* raw-mode read */
      return foodriver_dsp_read_data(cmd_port,&data,*count,
				     data_size,priority);
}
	...
	int		priority)		// priority of this transaction
{
	...
	msg = sndmsg_dsp_write_data(msg,
			      ...
			      bootImageSize);
	msg = sndmsg_dsp_host_flag(msg,FOODRIVER_ICR_HF0,FOODRIVER_ICR_HF0);
	msg = sndmsg_dsp_host_flag(msg,FOODRIVER_ICR_HF0,0);
	r = msg_rpc(msg, msg_rpc_options, MSG_SIZE_MAX, timeout, timeout);
	if (r != KERN_SUCCESS)
	  return r;
	r = snd_return_code((snd_illegal_msg_t *)msg);
 ...
 * Generic reset owner.
 */
static kern_return_t foodriver_reset_owner (
	port_t		device_port,	// valid device port
...
kern_return_t foodriver_reset_dsp_owner (
	...
	port_t		new_negotiation)// new dsp negotiation port
{
    return foodriver_reset_owner(device_port, old_owner_port, new_owner_port,
...
static kern_return_t foodriver_get_stream (
	port_t		device_port,	// valid device port
...
static kern_return_t foodriver_dsp_chandata (
	port_t		cmd_port,	// valid dsp command port
	...
	port_t		*stream_port)		// returned stream_port
{
    ...
    switch (config) {
	case FOODRIVER_STREAM_TO_SNDOUT_44:
	case FOODRIVER_STREAM_TO_SNDOUT_22:
	    if (sample_size != 2) return KERN_FAILURE;
	    if (config==FOODRIVER_STREAM_TO_SNDOUT_44)
		stream = SND_GD_SOUT_44;
	    else
		stream = SND_GD_SOUT_22;
	    r = foodriver_get_stream(dev_port,owner_port,stream_port,stream);
	    ...
	    r = foodriver_stream_ndma(*stream_port, 0, so_buf_count);
	    return r;
	case FOODRIVER_STREAM_FROM_SNDIN:
	    ...
	    put_stream_parms(*stream_port,low_water,high_water,
			     buf_size,sample_size);
	    return r;
	case FOODRIVER_DMA_STREAM_FROM_DSP:
	case FOODRIVER_STREAM_FROM_DSP:
	    ...
	    r = foodriver_get_dsp_cmd_port(dev_port,owner_port,&cmd_port);
	    if (r != KERN_SUCCESS) return r;
	    r = foodriver_dsp_chandata(cmd_port,0,buf_size,
	    ...
	    *protocol |= new_proto;
	    put_stream_parms(*stream_port,low_water,high_water,
			     buf_size,sample_size);
	    return KERN_SUCCESS;
	case FOODRIVER_DMA_STREAM_TO_DSP:
	case FOODRIVER_STREAM_TO_DSP:
	    /*** FIXME: should also support 3 bytes in some fashion ***/
	    if (sample_size != 1 && sample_size != 2 && sample_size != 4)
		return KERN_FAILURE;

	    if (config == FOODRIVER_DMA_STREAM_TO_DSP)
	      new_proto |= SND_DSP_PROTO_C_DMA;
	    else /* config == FOODRIVER_STREAM_TO_DSP */
	      new_proto |= SND_DSP_PROTO_SOUNDDATA;

	    r = foodriver_get_dsp_cmd_port(dev_port,owner_port,&cmd_port);
	    if (r != KERN_SUCCESS) return r;
	    r = foodriver_dsp_chandata(cmd_port,
	    ...
	    /* Pre 2.0: r = foodriver_dsp_chandata(cmd_port,0,buf_size,
	    ...
	    *protocol |= new_proto;
	    put_stream_parms(*stream_port,low_water,high_water,
			     buf_size,sample_size);
	    return KERN_SUCCESS;
	case FOODRIVER_STREAM_DSP_TO_SNDOUT_44:
	case FOODRIVER_STREAM_DSP_TO_SNDOUT_22:
	    if (sample_size != 2) return KERN_FAILURE;
	    new_proto |= SND_DSP_PROTO_LINKOUT|SND_DSP_PROTO_C_DMA;
	    if (config == FOODRIVER_STREAM_DSP_TO_SNDOUT_44) {
	    ...
	    *protocol |= new_proto;
		...
		foodriver_start_{reading,writing}.  A link is automatic, 
		so we do it here. */
	    ...
	    return r;
	case FOODRIVER_STREAM_THROUGH_DSP_TO_SNDOUT_22:
	case FOODRIVER_STREAM_THROUGH_DSP_TO_SNDOUT_44:
	    new_proto |= SND_DSP_PROTO_SOUNDDATA;
	case FOODRIVER_DMA_STREAM_THROUGH_DSP_TO_SNDOUT_22:
	case FOODRIVER_DMA_STREAM_THROUGH_DSP_TO_SNDOUT_44:
	    ...
	    if (sample_size != 1 && sample_size != 2 && sample_size != 4)
		return KERN_FAILURE;
	    if (config == FOODRIVER_STREAM_THROUGH_DSP_TO_SNDOUT_44 ||
		config == FOODRIVER_DMA_STREAM_THROUGH_DSP_TO_SNDOUT_44)
		new_proto |= SND_DSP_PROTO_HIGH;
	    r = foodriver_get_dsp_cmd_port(dev_port,owner_port,&cmd_port);
	    if (r != KERN_SUCCESS) return r;
	    /*
	     ...
	     */
	    r = foodriver_dsp_chandata(cmd_port,0,buf_size,0,0,2,DSP_SO_CHAN);
	    if (r != KERN_SUCCESS) return r;
	    r = foodriver_dsp_chandata(cmd_port,
				       0 /* dsp_buffer_address. p:0 means
					  memory space field holds channel# */,
	    ...
	    if (r != KERN_SUCCESS) return r;
	    /*
	     * Save options that foodriver_stream_start_writing() will use to
		...
		stream = SND_GD_SOUT_22;
	    r = foodriver_get_stream(dev_port,owner_port, &sndout_stream_port, stream);
	    if (r != KERN_SUCCESS) return r;
	    ...
	    *protocol |= new_proto;
	    return r;
	case FOODRIVER_STREAM_FROM_SNDIN_THROUGH_DSP:
	    new_proto |= SND_DSP_PROTO_C_DMA;
	    if (sample_size != 1 && sample_size != 2 && sample_size != 4)
	      return KERN_FAILURE;
	    r = foodriver_get_dsp_cmd_port(dev_port,owner_port,&cmd_port);
	    ...
	    *protocol |= new_proto;
	    put_stream_parms(*stream_port,low_water,high_water,
			     buf_size,sample_size);
	    return KERN_SUCCESS;
	case FOODRIVER_STREAM_SNDIN_TO_DSP:
	    /*
...
default:
	...
	{
#if DPRINT
	    fprintf(stderr,"\t(foodriver_dsp_dma_write: ill_msgid: %d)\n", r_msg.ill_msgid);
		fprintf(stderr,"\t(foodriver_dsp_dma_write: error: %d)\n", r_msg.ill_error);
#endif
		r = r_msg.ill_error;
	}
	return r;
	...
	pointer_t	*data)		// where data is put
{
	...
	while (done == 0)
	{
	    ...
	    {
#if DPRINT
		fprintf(stderr,"\t(foodriver_dsp_dma_read: ill_msgid: %d)\n", 
			s_rec.illegal.ill_msgid);
		fprintf(stderr,"\t(foodriver_dsp_dma_read: error: %d)\n", 
			s_rec.illegal.ill_error);
#endif
 ...
 */
kern_return_t foodriver_dspcmd_abortdma (
	port_t		cmd_port,	// valid dsp command port
 ...
 */
static kern_return_t foodriver_reply_server(
