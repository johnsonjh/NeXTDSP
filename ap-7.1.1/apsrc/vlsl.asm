...
;;  --------------------
;;	07/13/89/mtm - created from vlsr
;;      
;;  ------------------------------ DOCUMENTATION ---------------------------
;;  NAME
;;      vlsl (AP macro) - vector logical shift left
;;          - shift left (sign ignored) each element of vector
;;      
;;  SYNOPSIS
;;      include 'ap_macros' ; load standard DSP macro package
;;      vlsl pf,ic,sinp,ainp0,iinp0,sout,aout0,iout0,cnt0 ; invoke ap macro vlsl
;;      
;;  MACRO ARGUMENTS
;;      pf      =   global label prefix (any text unique to invoking macro)
;;      ic      =   instance count (such that pf\_vlsl_\ic is globally unique)
...
;;      
;;  DESCRIPTION
;;      The vlsl array-processor macro left-shifts every element of 
...
;;      vlsl (prefix)pf,(instance)ic,
;;          (dspace)sinp,(input)ainp,iinp,
;;          (dspace)sout,(output)aout,iout,cnt
;;          
;;  DSPWRAP C SYNTAX
;;      ierr    =   DSPAPvlsl(S,iS,D,iD,n);   /* vector logical shift left
...
;;      beg_ap  'tvlsl'             ; begin ap main program
;;      new_xeb out1vec,12,-1
;;      new_yeb y1vec,12,0
;;      vlsl    ap,1,x,out1vec,1,y,y1vec,1,12
;;      end_ap  'tvlsl'             ; end of ap main program
;;      
;;  SOURCE
;;      /usr/lib/dsp/apsrc/vlsl.asm
;;      
;;  REGISTER USE
;;      A       working register
;;      B       working register
...
;;      
vlsl    macro pf,ic,sinp,ainp0,iinp0,sout,aout0,iout0,cnt0

        ; argument declarations
        new_xarg    pf\_vlsl_\ic\_,ainp,ainp0   ; input address arg
        new_xarg    pf\_vlsl_\ic\_,iinp,iinp0   ; in address incr arg
        new_xarg    pf\_vlsl_\ic\_,aout,aout0   ; out address arg
        new_xarg    pf\_vlsl_\ic\_,iout,iout0   ; out address incr arg
        new_xarg    pf\_vlsl_\ic\_,cnt,cnt0     ; element count arg
...
pf\_vlsl_\ic\_l3                            ; jump to here for cnt=0 or cnt=1
        ...
        tst     B
        jeq     pf\_vlsl_\ic\_l2            ; if cnt odd (including cnt=1),
