...
;;  ------------------------------ DOCUMENTATION ---------------------------
;;  NAME
;;      vmv (AP macro) - vector minus vector 
;;          - subtract two vectors, creating a third
;;      
;;  SYNOPSIS
;;      include 'ap_macros' ; load standard DSP macro package
;;      vmv     pf,ic,sinp_a,ainp_a0,iinp_a0,sinp_b,ainp_b0,iinp_b0,sout,aout0,iout0,cnt0
;;      
;;  MACRO ARGUMENTS
;;      pf      =   global label prefix (any text unique to invoking macro)
;;      ic      =   instance count (such that pf\_vmv_\ic is globally unique)
...
;;      
;;  DESCRIPTION
;;      The vmv array-processor macro computes C=A-B.
;;
;;  PSEUDO-C NOTATION
;;      ainp_a  =   x:(R_X)+;    
;;      iinp_a  =   x:(R_X)+;    
...
;;          +1 program memory location if sinp_a==sout
;;
;;  EXECUTION TIME 
...
;;      vmv    ap,1,x,ax_vec,1,x,bx_vec,1,x,out1_vec,1,12
;;      end_ap  'tvmv'
;;      end
;;      
;;  SOURCE
;;      /usr/lib/dsp/apsrc/vmv.asm
;;      
;;  REGISTER USE 
;;      A       accumulator; temp for testing for cnt==0
;;      B       term from B matrix
;;      X0      cnt
...
;;      
vmv    macro pf,ic,sinp_a,ainp_a0,iinp_a0,sinp_b,ainp_b0,iinp_b0,sout,aout0,iout0,cnt0

        ; argument definitions
        new_xarg    pf\_vmv_\ic\_,ainp_a,ainp_a0
        new_xarg    pf\_vmv_\ic\_,iinp_a,iinp_a0
        new_xarg    pf\_vmv_\ic\_,ainp_b,ainp_b0
        new_xarg    pf\_vmv_\ic\_,iinp_b,iinp_b0
        new_xarg    pf\_vmv_\ic\_,aout,aout0
        new_xarg    pf\_vmv_\ic\_,iout,iout0
        new_xarg    pf\_vmv_\ic\_,cnt,cnt0
        ...
        jeq     pf\_vmv_\ic\_loop1     ; protect againt count=0

        if "sinp_a"=="sinp_b"
            move            sinp_a:(R_I1)+N_I1,A
            ...
            if "sinp_a"=='x'
                ...
                move            sinp_a:(R_I1)+N_I1,A    sinp_b:(R_I2)+N_I2,B
            else
                move            sinp_b:(R_I2)+N_I2,B    sinp_a:(R_I1)+N_I1,A
            endif
        endif          

        ; inner loop
        do      X0,pf\_vmv_\ic\_loop1
            sub     B,A     sinp_b:(R_I2)+N_I2,B
            if "sinp_a"=="sout" 
                move            A,sout:(R_O)+N_O    
                move            sinp_a:(R_I1)+N_I1,A
            else
                if "sinp_a"=='x'
                    move            sinp_a:(R_I1)+N_I1,A    A,sout:(R_O)+N_O
                else
                    move            A,sout:(R_O)+N_O        sinp_a:(R_I1)+N_I1,A
                endif
            endif
pf\_vmv_\ic\_loop1
