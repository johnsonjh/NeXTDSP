...
;;  --------------------
;;      09/20/87/jms - initial file created from DSPAPSRC/VMAX.ASM
;;      02/23/88/jms - cosmetic changes to code and documentation
;;      03/29/88/jms - fix bugs in CALLING DSP PROGRAM TEMPLATE
;;      
;;  ------------------------------ DOCUMENTATION ---------------------------
;;  NAME
;;      vmin (AP macro) - vector min - min two vectors, creating a third
;;      
;;  SYNOPSIS
;;      include 'ap_macros' ; load standard DSP macro package
;;      vmin    pf,ic,sinp_a,ainp_a0,iinp_a0,sinp_b,ainp_b0,iinp_b0,sout,aout0,iout0,cnt0
;;      
;;  MACRO ARGUMENTS
;;      pf      =   global label prefix (any text unique to invoking macro)
;;      ic      =   instance count (such that pf\_vmin_\ic is globally unique)
...
;;      
;;  DESCRIPTION
;;      The vmin array-processor macro computes C=A MIN B.
;;
;;  PSEUDO-C NOTATION
;;      ainp_a  =   x:(R_X)+;
;;      iinp_a  =   x:(R_X)+;
...
;;      for (n=0;n<cnt;n++) 
;;          sout:aout[n*iout] = 
;;              sinp_a:ainp_a[n*iinp_a] MIN sinp_b:ainp_b[n*iinp_b];
;;      
;;  MEMORY USE 
...
;;      vmin    ap,1,x,ax_vec,1,x,bx_vec,1,x,out1_vec,1,12
;;      end_ap  'tvmin'
;;      end
;;      
;;  SOURCE
;;      /usr/lib/dsp/apsrc/vmin.asm
;;      
;;  REGISTER USE 
...
;;      
vmin    macro pf,ic,sinp_a,ainp_a0,iinp_a0,sinp_b,ainp_b0,iinp_b0,sout,aout0,iout0,cnt0

        ; argument definitions
        new_xarg    pf\_vmin_\ic\_,ainp_a,ainp_a0
        new_xarg    pf\_vmin_\ic\_,iinp_a,iinp_a0
        new_xarg    pf\_vmin_\ic\_,ainp_b,ainp_b0
        new_xarg    pf\_vmin_\ic\_,iinp_b,iinp_b0
        new_xarg    pf\_vmin_\ic\_,aout,aout0
        new_xarg    pf\_vmin_\ic\_,iout,iout0
        new_xarg    pf\_vmin_\ic\_,cnt,cnt0
...
pf\_vmin_\ic\_l1     
        ...
        do      Y0,pf\_vmin_\ic\_l3
            ...
            tgt     X0,A
pf\_vmin_\ic\_l3                        ; jump to here for cnt=0 or cnt=1
        move    Y1,B
        tst     B    
        jeq     pf\_vmin_\ic\_l2            ; if cnt odd (including cnt=1),
